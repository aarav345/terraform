...
# frontend
resource "aws_codedeploy_app" "frontend_deploy" {
...
resource "aws_codedeploy_deployment_group" "frontend_deployment_group" {
    app_name = aws_codedeploy_app.frontend_deploy.name
    deployment_group_name = "${var.project_name}-frontend-deployment-group"
    service_role_arn = data.aws_iam_role.codedeploy_role.arn
    ...
    # Associate automatic scaling group
    autoscaling_groups = [
        aws_autoscaling_group.presentation_tier_asg.name
    ]

    # Load balancer settings
    load_balancer_info {
        target_group_info {
            name = aws_lb_target_group.presentation_tg.name
        }
    }
